    ...
    try:
        ...
        if request.method == 'GET': 
            ...
            if attempt is not None:
                ...
                db.commit()  
                messageactivate = 'User was activated. please login'
                flash(messageactivate)
    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                ...
                db.commit()
                mensajecambio='Updated password. please login'
                flash(mensajecambio)
                return redirect(url_for('auth.login'))
            else:
                flash('Invalid')
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'select id from forgotlink WHERE challenge=? and state=? and CURRENT_TIMESTAMP BETWEEN created AND validuntil ', (number, utils.F_ACTIVE)
            ).fetchone()
